# apiserverproxy

Kubernetes API Server 透明代理，支持多集群、Watch 流式请求和本地缓存。

## 功能特性

- **多集群代理**：通过 config.json 配置多个集群，请求路径自动路由
- **SDK 兼容**：支持 Kubernetes Python SDK、Java SDK、client-go 等官方 SDK 直接接入
- **Watch 支持**：自动检测 `?watch=true`，使用 chunked 流式传输
- **本地缓存**：Pod 和 Service 列表请求走本地缓存，减少 API server 压力,保护apiserver和etcd
- **labelSelector / fieldSelector**：完整支持 Kubernetes 原生过滤语法
- **配置热加载**：修改 config.json 后自动更新集群缓存，无需重启
- **请求跟踪**：自动生成或透传 X-Trace-Id，访问日志包含 trace_id 字段
- **日志轮转**：支持 lumberjack 日志轮转，可配置文件大小、备份数量和保留天数

## 快速开始

### 1. 构建

```bash
make build
```

### 2. 配置

创建 `config.json`：

```json
{
  "log": {
    "level": "info",
    "filePath": "logs/apiserverproxy.log",
    "maxSizeMB": 100,
    "maxBackups": 5,
    "maxAgeDays": 30
  },
  "clusters": [
    {
      "name": "cluster1",
      "server": "https://10.0.0.1:6443",
      "token": "eyJhbGciOiJSUzI1NiIs..."
    },
    {
      "name": "cluster2",
      "server": "https://10.0.0.2:6443",
      "token": "eyJhbGciOiJSUzI1NiIs..."
    }
  ]
}
```

获取 token：

```bash
kubectl create token <service-account> -n default --duration=24h
```

### 3. 启动

```bash
./bin/apiserverproxy --config-file=config.json --listen=:8080
```

## 路由格式

代理支持两种路由格式：

| 格式 | 路径 | 用途 |
|------|------|------|
| 原有模式 | `/{cluster}/api/v1/pods` | curl / HTTP 客户端 |
| SDK 模式 | `/clusters/{cluster}/api/v1/pods` | K8s SDK (client-go / Python / Java) |

两种格式功能完全一致，SDK 模式是为了兼容 Kubernetes 官方 SDK 的 URL 拼接规则。

## 使用示例

### curl / HTTP 客户端

```bash
# 列出所有 Pod（走缓存）
curl http://localhost:8080/cluster1/api/v1/pods

# 指定命名空间
curl http://localhost:8080/cluster1/api/v1/namespaces/default/pods

# 使用 labelSelector
curl "http://localhost:8080/cluster1/api/v1/pods?labelSelector=app=nginx"

# 使用 fieldSelector
curl "http://localhost:8080/cluster1/api/v1/pods?fieldSelector=spec.nodeName=node1"

# 列出 Service（走缓存）
curl http://localhost:8080/cluster1/api/v1/services

# Watch 请求（不走缓存）
curl -N "http://localhost:8080/cluster1/api/v1/pods?watch=true"

# 其他资源（直接代理）
curl http://localhost:8080/cluster1/api/v1/namespaces
curl http://localhost:8080/cluster1/apis/apps/v1/deployments
```

### Kubernetes Python SDK

安装：

```bash
pip install kubernetes
```

使用：

```python
from kubernetes import client, config
from kubernetes import watch

# 配置代理地址（只读操作无需配置 token）
configuration = client.Configuration()
configuration.host = "http://localhost:8080/clusters/cluster1"
configuration.verify_ssl = False  # 代理使用 HTTP

api = client.CoreV1Api(client.ApiClient(configuration))

# 列出所有 Pod（GET 请求，使用代理默认 token）
pods = api.list_pod_for_all_namespaces()
for pod in pods.items:
    print(f"{pod.metadata.namespace}/{pod.metadata.name} ({pod.status.phase})")

# 列出指定命名空间的 Service
services = api.list_namespaced_service("default")
for svc in services.items:
    print(f"{svc.metadata.name} ({svc.spec.type})")

# 使用 labelSelector 过滤
pods = api.list_pod_for_all_namespaces(label_selector="app=nginx")

# 使用 fieldSelector 过滤
pods = api.list_pod_for_all_namespaces(field_selector="status.phase=Running")

# Get 单个 Pod
pod = api.read_namespaced_pod("my-pod", "default")

# Watch
w = watch.Watch()
for event in w.stream(api.list_pod_for_all_namespaces, timeout_seconds=60):
    print(f"Event: {event['type']} {event['object'].metadata.name}")
```

写操作需要配置具有权限的 token：

```python
# 写操作配置（create / update / delete）
configuration = client.Configuration()
configuration.host = "http://localhost:8080/clusters/cluster1"
configuration.api_key = {"authorization": "Bearer <your-token>"}
configuration.verify_ssl = False

api = client.CoreV1Api(client.ApiClient(configuration))

# 创建 Namespace
ns = client.V1Namespace(metadata=client.V1ObjectMeta(name="test"))
api.create_namespace(ns)

# 删除 Namespace
api.delete_namespace("test")
```

### Kubernetes Java SDK

Maven 依赖：

```xml
<dependency>
    <groupId>io.kubernetes</groupId>
    <artifactId>client-java</artifactId>
    <version>22.0.0</version>
</dependency>
```

使用：

```java
import io.kubernetes.client.openapi.ApiClient;
import io.kubernetes.client.openapi.ApiException;
import io.kubernetes.client.openapi.Configuration;
import io.kubernetes.client.openapi.apis.CoreV1Api;
import io.kubernetes.client.openapi.models.V1PodList;
import io.kubernetes.client.openapi.models.V1ServiceList;
import io.kubernetes.client.util.Config;

ApiClient client = Config.defaultClient();

// 配置代理地址（只读操作无需配置 token）
client.setBasePath("http://localhost:8080/clusters/cluster1");
client.setVerifyingSsl(false);  // 代理使用 HTTP

Configuration.setDefaultClient(client);
CoreV1Api api = new CoreV1Api();

// 列出所有 Pod（GET 请求，使用代理默认 token）
V1PodList pods = api.listPodForAllNamespaces()
    .execute();
pods.getItems().forEach(pod ->
    System.out.printf("%s/%s (%s)%n",
        pod.getMetadata().getNamespace(),
        pod.getMetadata().getName(),
        pod.getStatus().getPhase())
);

// 列出 Service
V1ServiceList services = api.listNamespacedService("default")
    .execute();

// 使用 labelSelector 过滤
pods = api.listPodForAllNamespaces()
    .labelSelector("app=nginx")
    .execute();

// 使用 fieldSelector 过滤
pods = api.listPodForAllNamespaces()
    .fieldSelector("status.phase=Running")
    .execute();
```

写操作需要配置具有权限的 token：

```java
// 写操作配置（create / update / delete）
ApiClient writeClient = Config.defaultClient();
writeClient.setBasePath("http://localhost:8080/clusters/cluster1");
writeClient.setApiKey("Bearer <your-token>");
writeClient.setVerifyingSsl(false);

Configuration.setDefaultClient(writeClient);
CoreV1Api writeApi = new CoreV1Api();
```

### client-go

```go
import (
    "k8s.io/client-go/kubernetes"
    "k8s.io/client-go/rest"
    metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
)

// 只读操作无需配置 token
cfg := &rest.Config{
    Host: "http://localhost:8080/clusters/cluster1",
    TLSClientConfig: rest.TLSClientConfig{
        Insecure: true,
    },
}

client, err := kubernetes.NewForConfig(cfg)

// 列出所有 Pod（GET 请求，使用代理默认 token）
pods, err := client.CoreV1().Pods("").List(ctx, metav1.ListOptions{})

// 列出指定命名空间的 Service
services, err := client.CoreV1().Services("default").List(ctx, metav1.ListOptions{})

// 使用 labelSelector 过滤
pods, err := client.CoreV1().Pods("").List(ctx, metav1.ListOptions{
    LabelSelector: "app=nginx",
})

// Watch
watcher, err := client.CoreV1().Pods("").Watch(ctx, metav1.ListOptions{})
for event := range watcher.ResultChan() {
    pod := event.Object.(*corev1.Pod)
    fmt.Printf("%s %s/%s\n", event.Type, pod.Namespace, pod.Name)
}
```

写操作需要配置具有权限的 token：

```go
// 写操作配置（create / update / delete）
writeCfg := &rest.Config{
    Host:        "http://localhost:8080/clusters/cluster1",
    BearerToken: "<your-token>",
    TLSClientConfig: rest.TLSClientConfig{
        Insecure: true,
    },
}
writeClient, _ := kubernetes.NewForConfig(writeCfg)
```

## 缓存说明

缓存命中时响应头包含 `X-Cache: HIT`。

| 请求类型 | 是否走缓存 |
|---------|-----------|
| `GET /api/v1/pods` | 是 |
| `GET /api/v1/services` | 是 |
| `GET /api/v1/pods?watch=true` | 否 |
| `GET /api/v1/namespaces` | 否 |
| `POST/PUT/DELETE` | 否 |

支持的 fieldSelector：

| 资源 | 支持的字段 |
|------|-----------|
| Pod | metadata.name、spec.nodeName、status.phase |
| Service | metadata.name |

## 认证说明

代理根据 HTTP 方法采用不同的认证策略：

| HTTP 方法 | 认证策略 | 说明 |
|-----------|---------|------|
| GET | 使用 config.json 中的默认 token | 客户端的 Authorization header 被忽略 |
| POST / PUT / DELETE / PATCH | 使用客户端自带的 token | 必须携带 Authorization header，否则返回 401 |

### 行为示例

```bash
# GET 请求：无需携带 token，代理使用 config 默认 token
curl http://localhost:8080/cluster1/api/v1/pods

# GET 请求：即使携带了伪造 token 也会被忽略，仍使用 config token
curl -H "Authorization: Bearer fake-token" http://localhost:8080/cluster1/api/v1/pods

# POST 请求：必须携带有效 token
curl -X POST -H "Authorization: Bearer <your-token>" \
  -H "Content-Type: application/json" \
  -d '{"apiVersion":"v1","kind":"Namespace","metadata":{"name":"test"}}' \
  http://localhost:8080/cluster1/api/v1/namespaces

# POST 请求：不携带 token 返回 401
curl -X POST http://localhost:8080/cluster1/api/v1/namespaces
# {"error":"non-GET requests require Authorization header"}
```

### SDK 使用说明

对于只读操作（list / get / watch），SDK 无需配置 token，代理会自动使用默认身份。对于写操作（create / update / delete / patch），SDK 必须配置具有相应权限的 token。

## 配置热加载

修改 config.json 后，代理会自动检测变更并更新集群缓存，无需重启：

- **新增集群**：自动创建缓存
- **移除集群**：自动停止缓存
- **变更集群**（server 或 token 变化）：自动重启缓存

日志示例：

```
time="2026-06-17 10:00:01" level=info msg="config reloaded, 3 clusters"
time="2026-06-17 10:00:01" level=info msg="starting cache for new cluster: staging"
time="2026-06-17 10:00:01" level=info msg="stopping cache for removed cluster: dev"
```

## 请求跟踪

每个请求都会记录访问日志，包含 trace_id 字段：

```
time="2026-06-17 10:00:01" level=info msg="GET /cluster1/api/v1/pods 200 12ms" trace_id=a1b2c3d4e5f6...
```

- 请求头带 `X-Trace-Id`：直接使用，透传到上游 K8s API
- 请求头不带 `X-Trace-Id`：自动生成 128-bit ID，响应头返回

```bash
# 使用自定义 trace-id
curl -H "X-Trace-Id: my-trace-123" http://localhost:8080/cluster1/api/v1/pods

# 自动生成 trace-id
curl http://localhost:8080/cluster1/api/v1/pods
# 响应头包含: X-Trace-Id: a1b2c3d4...
```

## 日志配置

| 参数 | 说明 | 默认值 |
|------|------|--------|
| level | 日志级别：debug, info, warn, error | info |
| filePath | 日志文件路径，为空则只输出 stdout | 空 |
| maxSizeMB | 单个日志文件最大大小（MB） | 100 |
| maxBackups | 保留的旧日志文件数量 | 5 |
| maxAgeDays | 旧日志文件保留天数 | 30 |

## 容器部署

### 构建镜像

```bash
docker build -t apiserverproxy:latest .
```

### Kubernetes 部署

```bash
# 编辑 deploy/configmap.yaml，填入集群配置
kubectl apply -f deploy/configmap.yaml
kubectl apply -f deploy/deployment.yaml

# 更新配置后自动热加载，无需重启
# 如需手动重启
kubectl rollout restart deployment/apiserverproxy
```

## 命令行参数

```
--config-file    集群配置文件路径（必填）
--listen         监听地址（默认 :8080）
```
